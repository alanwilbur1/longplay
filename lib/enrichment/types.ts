...
export interface EnrichmentRoundStats {
  /** Number of candidate rows the SELECT pulled (status in
   *  queued/failed) before the runnable filter. */
  queued: number
  /** Number of candidates that passed the isJobRunnable filter and
   *  were eligible to run this round. */
  selected: number
  /** Of selected, how many we actually attempted (bounded by
   *  maxJobs, timeoutMs, and the rate-limit short-circuit). */
  run: number
  succeeded: number
  failed: number
  ...
  /** Resolved default provider at runtime ('lastfm' | null) — null
   *  means no API key configured, in which case the round is a no-op. */
  provider_resolved: EnrichmentProvider | null
  ...
  last_error: string | null
  /** Set when the round runner's initial SELECT against
   *  artist_genre_enrichments failed (typically: migration not
   *  applied). Includes the Supabase error code + message. */
  db_error: string | null
}

/** Per-batch enqueue stats. */
export interface EnrichmentEnqueueBatchStats {
  attempted: number
  /** Newly inserted rows. */
  inserted: number
  /** Already-existing rows (we don't reset status). */
  existed: number
  /** DB-level failures. */
  failed: number
  /** First error message we encountered, for diagnostic surfacing. */
